...
import { operacaoSchema } from "@/schemas/picklists/operacaoSchema";
...
export default function ConfiguracoesLojas() {
  ...
  async function save() {
    try {
      ...
      const response = await fetch("/api/configuracoes/picklists/operacoes", {
        method: "POST",
        headers: {
  ...
  async function update() {
    try {
      ...
      const response = await fetch(
        `/api/configuracoes/picklists/operacoes/?id=${id}/`,
        {
          method: "PUT",
          headers: {
            Authorization: session?.user?.token,
  ...
  const columns = [
    ...
    {
      field: "name",
      headerName: "NOME DA OPERAÇÃO",
      renderHeader: (params) => <strong>NOME DA OPERAÇÃO</strong>,
      minWidth: 150,
      align: "center",
      headerAlign: "center",
    },
  ...
  return (
    <ContentWrapper title="Cadastro e manutenção de operações">
