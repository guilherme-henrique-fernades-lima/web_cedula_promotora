...
import { promotoraSchema } from "@/schemas/picklists/promotoraSchema";

export default function ConfiguracoesCanalAquisicaoClientes() {
  ...
  const {
    ...
    mode: "onChange",
    resolver: yupResolver(promotoraSchema),
  });
  ...
  async function save() {
    try {
      ...
      const response = await fetch(
        "/api/configuracoes/picklists/canal-aquisicao-clientes",
        {
          method: "POST",
          headers: {
  ...
  async function update() {
    try {
      ...
      const response = await fetch(
        `/api/configuracoes/picklists/canal-aquisicao-clientes/?id=${id}/`,
        {
          method: "PUT",
          headers: {
            Authorization: session?.user?.token,
  ...
  return (
    <ContentWrapper title="Cadastro e manutenção de canais de aquisição de clientes">
        ...
        <Grid container spacing={1} sx={{ mt: 1 }}>
          <Grid item xs={12} sm={6} md={4} lg={4} xl={3}>
            <TextField
              ...
              size="small"
              label="Nome"
              placeholder="Insira o nome da promotora"
