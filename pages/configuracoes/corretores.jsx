...
import { corretorSchema } from "@/schemas/picklists/corretorSchema";
...
export default function ConfiguracoesLojas() {
  ...
  const {
    ...
    mode: "onChange",
    resolver: yupResolver(corretorSchema),
  });
  ...
  async function save() {
    try {
      ...
      const response = await fetch("/api/configuracoes/picklists/corretores", {
        method: "POST",
        headers: {
  ...
  async function update() {
    try {
      ...
      const response = await fetch(
        `/api/configuracoes/picklists/corretores/?id=${id}/`,
        {
          method: "PUT",
          headers: {
            Authorization: session?.user?.token,
  ...
  const columns = [
    ...
    {
      field: "name",
      headerName: "NOME DO CORRETOR",
      renderHeader: (params) => <strong>NOME DO CORRETOR</strong>,
      minWidth: 250,
      align: "center",
      headerAlign: "center",
    },
  ...
  return (
    <ContentWrapper title="Cadastro e manutenção de corretores">
        ...
        <Grid container spacing={1} sx={{ mt: 1 }}>
          <Grid item xs={12} sm={6} md={4} lg={4} xl={3}>
            <TextField
              ...
              size="small"
              label="Nome"
              placeholder="Insira o nome do(a) corretor(a)"
