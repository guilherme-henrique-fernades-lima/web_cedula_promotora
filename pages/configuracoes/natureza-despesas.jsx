...
import { naturezaDespesaSchema } from "@/schemas/picklists/naturezaDespesaSchema";
...
export default function ConfiguracoesLojas() {
  ...
  const {
    ...
    mode: "onChange",
    resolver: yupResolver(naturezaDespesaSchema),
  });
  ...
  async function save() {
    try {
      ...
      const response = await fetch(
        "/api/configuracoes/picklists/natureza-despesas",
        {
          method: "POST",
          headers: {
  ...
  async function update() {
    try {
      ...
      const response = await fetch(
        `/api/configuracoes/picklists/natureza-despesas/?id=${id}/`,
        {
          method: "PUT",
          headers: {
            Authorization: session?.user?.token,
  ...
  return (
    <ContentWrapper title="Cadastro e manutenção de natureza de despesas">
        ...
        <Grid container spacing={1} sx={{ mt: 1 }}>
          <Grid item xs={12} sm={6} md={4} lg={4} xl={3}>
            <TextField
              ...
              size="small"
              label="Nome"
              placeholder="Insira o nome da natureza da operação"
